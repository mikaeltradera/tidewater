...
ok( defaultCrabTraps().length === 8 && defaultCrabTraps().every( ( trap ) => trap.state === 'stored' && trap.crabs === 0 ) && defaultCrabTraps().filter( ( trap ) => trap.cache === 'pier' ).length === 4 && defaultCrabTraps().filter( ( trap ) => trap.cache === 'beach' ).length === 4, 'a new save starts with four empty traps at each supply stack' );
